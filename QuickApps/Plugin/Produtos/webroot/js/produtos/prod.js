$(function(){
			...
			$.ajax({
				...
				success:function(resposta)
				{
					for(indice in resposta)
						ProdutosProdutoLinhaId.append('<option '+(id_linha != 0 && indice == id_linha ? 'selected' : "")+' value="'+indice+'">'+resposta[indice]+'</option>');
				}
			});
		}
	}).trigger('change');
	...
	$("#btnAddSku").click(function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				var tmp_tpl = tpl.replace(/{COLUMN_1}/g, input_ref.val()).replace(/{COLUMN_COR}/g, input_cor.val()).replace(/{COLUMN_2}/g, input_site.find(":selected").text()).replace(/{COLUMN_3}/g, input_celular.find(":selected").text()).replace(/{ID}/g, data.product_sku_id);
	...
	var money_options = { decimal:",", thousands:"."};
	var v1 = $('#ProdutosPreco1').val();
	$('#ProdutosPreco1').maskMoney(money_options).val(v1);
	var v2 = $('#ProdutosPreco2').val();
	$('#ProdutosPreco2').maskMoney(money_options).val(v2);
});
